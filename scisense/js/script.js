...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Submit Feedback Function
  function submitFeedback(userId, demographic, stimuliFeedback, kGainFeedback) {
    db.collection('feedback').add({
      userId: userId,
      demographic: demographic, // { age, academicLevel, occupation }
      stimuliFeedback: stimuliFeedback, // { newsVsAbstracts, news1VsNews2 }
  ...
  feedbackForm.addEventListener('submit', (e) => {
    ...
    // Submit Feedback to Firestore
    submitFeedback(user.uid, demographic, stimuliFeedback, kGainFeedback);
  ...
  // Optional: Load initial content (e.g., trending papers)
  ...
  for (let i = 2; i <= 10; i++) {
    ...
    paperItem.innerHTML = `
      ...
      <h3 class="paper-title">Sample Paper Title ${i}</h3>
      <p class="paper-info">Author ${i}, Co-author • Jan ${i}, 2024</p>
      <span class="category-tag">cs.AI</span>
      <span class="engagement">${i * 5} Comments</span>
    `;
...
if (window.location.pathname.endsWith('paper.html') && paperId) {
  db.collection('feedback')
    .where('paperId', '==', paperId)
    .onSnapshot((querySnapshot) => {
      const engagementSpan = document.querySelector('.engagement');
      engagementSpan.textContent = `${querySnapshot.size} Comments`;
    }, (error) => {
